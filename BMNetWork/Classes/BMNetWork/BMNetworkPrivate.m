...
void BMLog(NSString *format, ...) {
#ifdef DEBUG
    if (![BMNetworkConfig sharedConfig].debugLogEnabled) {
        return;
    }
    va_list argptr;
...
+ (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        BMLog(@"error to set do not backup attribute, error = %@", error);
    }
}
...
@implementation BMRequest (RequestAccessory)

- (void)toggleAccessoriesWillStartCallBack {
    for (id<BMRequestAccessory> accessory in self.requestAccessories) {
...
@implementation BMBatchRequest (RequestAccessory)
